...
import { Play, ShoppingCart, CheckCircle, Loader } from 'lucide-react';
  ...
  const renderActionButton = () => {
    if (isPurchased) {
      return (
        ...
        >
          <button
            className="w-full font-semibold py-2 xxs:py-3 px-4 xxs:px-6 rounded-lg transition-all duration-500 ease-in-out transform hover:scale-110 hover:-translate-y-1 shadow-xl hover:shadow-2xl flex items-center justify-center space-x-1 xxs:space-x-2 text-sm xxs:text-base border bg-green-600 hover:bg-green-700 text-white border-green-500"
          >
            <Play className="h-4 w-4 xxs:h-5 xxs:w-5" />
            <span>{t('course_card.view_course', 'View Course')}</span>
          </button>
        </Link>
      );
    }
      ...
      <div className="relative overflow-hidden h-36 xxs:h-40 sm:h-44 shadow-inner">
        ...
        {isPurchased ? (
          <div className="absolute inset-0 bg-gradient-to-br from-black/50 via-black/40 to-black/50 bg-opacity-0 group-hover:bg-opacity-100 transition-all duration-500 ease-out flex items-center justify-center">
           {/* <div className="flex flex-wrap gap-2 xxs:gap-3 mb-3 xxs:mb-3.5">
              {tags && tags.slice(0, 3).map((_tag, index) => (
                <span
                  key={index}
            ...
            >
              <Play className="h-4 w-4 xxs:h-5 xxs:w-5" />
              <span>{t('course_card.view_course')}</span>
            </Link>
          </div>
        ) : (
          <div className="absolute inset-0 bg-gradient-to-br from-black/50 via-black/40 to-black/50 bg-opacity-0 group-hover:bg-opacity-100 transition-all duration-500 ease-out flex items-center justify-center">
      ...
      <div className="p-3 xxs:p-4 pb-2 xxs:pb-3 flex flex-col flex-grow bg-gray-800 sm:bg-gradient-to-b sm:from-gray-800 sm:to-gray-900/30">
        ...
        <div className="flex items-center justify-between mb-2 xxs:mb-2.5 mt-auto">
          <div className="flex items-center space-x-1 text-gray-400 text-xs xxs:text-sm">
            <span>{lessons || 0} {t('course_card.lessons')}</span>
          </div>
