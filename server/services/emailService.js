...
class EmailService {
   ...
   * @returns {Promise<{success: number, failed: number, errors: Array}>} - Send results
   */
  async sendBulkEmail(emails, subject, message, isHtml = true) {
    const results = {
      success: 0,
      failed: 0,
      errors: []
    ...
    if (!this.isConfigured || !this.transporter) {
      console.log('⚠️  Email service not configured - cannot send bulk email');
      return results;
    }
    ...
    console.log(`✅ Bulk email sent: ${results.success} successful, ${results.failed} failed`);
    return results;
  }
